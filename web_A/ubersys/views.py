...
from folium import  ClickForMarker,ClickForLatLng,LatLngPopup,LayerControl
from folium.plugins import Geocoder,LocateControl,Search,Fullscreen,Draw ,TimestampedGeoJson
import asyncio
...
def mapy(request):
    ...
    m1 = folium.Map(location=[28.644800, 77.216721],zoom_start=18);m1._id = "map";m1._name = "_";m1.add_child(folium.ClickForLatLng(format_str='"[" + lat + "," + lng + "]"', alert=False))

    LatLngPopup().add_to(m1);Fullscreen().add_to(m1);Geocoder().add_to(m1);LatLngPopup().add_to(m1);aa=LocateControl();aa._id='gps';aa._name="getlocation";aa.add_to(m1)
    ...
    m1.get_root().header.add_child(folium.JavascriptLink('/static/js/ubs/ubs.js'))
    m1.get_root().html.add_child(folium.CssLink('/static/js/ubs/ubs.css'))
    m1.get_root().html.add_child(folium.JavascriptLink('https://unpkg.com/htmx.org@1.9.4/dist/htmx.min.js'))

    m1.get_root().header.add_child(folium.JavascriptLink('https://unpkg.com/htmx.org@1.9.4/dist/htmx.min.js'))
    m1.get_root().html.add_child(folium.JavascriptLink('https://unpkg.com/htmx.org/dist/ext/ws.js'))
