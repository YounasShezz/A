...
class Profile(models.Model):
    ...
    jobs = models.OneToOneField(Type_job,blank=True,on_delete=models.CASCADE)
    def __str__(self):
...
class Catigory(models.Model):
    ...
    code = models.CharField(max_length=50)
    def insert(self):
        s =Catigory.objects.all()
        for ss in s:
            self.choice.insert(0,f"{ss.cat}",f"{ss.cat}")
    def __str__(self):
...
class Prodect(models.Model):
    prodect = models.OneToOneField(Type_prodect)
    catigory = models.ForeignKey(Type_prodect,on_delete=models.CASCADE) 
...
class Chop_prodect(models.Model):
    ...
    type_shop_prodect = models.ForeignKey(Type_prodect,on_delete=models.CASCADE)          
